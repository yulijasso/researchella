...
// Dynamically import canvas to avoid build issues on Vercel
...
async function getCreateCanvas() {
  if (!createCanvas) {
    try {
      ...
      console.warn('Canvas not available, OCR features disabled');
      return null;
    }
  }
  return createCanvas;
...
export async function pdfPageToImage(page) {
  ...
  if (!createCanvasFn) {
    throw new Error('Canvas not available for PDF processing');
  }
