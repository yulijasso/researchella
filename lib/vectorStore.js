...
export async function addDocumentsToStore(documents, sessionId = 'default', userId = null, waitForIndexing = true) {
  ...
  if (waitForIndexing && langchainDocs.length > 0) {
    ...
    const POLL_INTERVAL_MS = 800; // 800ms between polls
    const sampleContent = langchainDocs[0].pageContent.substring(0, 50);

    console.log(`⏳ Waiting for Pinecone to index ${langchainDocs.length} vectors...`);

    for (let attempt = 1; attempt <= MAX_WAIT_ATTEMPTS; attempt++) {
      await new Promise(resolve => setTimeout(resolve, POLL_INTERVAL_MS));

      try {
        // Try to search for the just-added document
        const filter = { sessionId, userId };
        const results = await store.similaritySearchWithScore(sampleContent, 1, filter);

        if (results.length > 0) {
          console.log(`✅ Pinecone indexed vectors after ${attempt * POLL_INTERVAL_MS}ms`);
          break;
        } else {
      ...
      if (attempt === MAX_WAIT_ATTEMPTS) {
        console.log(`⚠️ Max wait time reached. Vectors may take a few more seconds to become searchable.`);
      }
    }
  }
