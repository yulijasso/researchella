    ...
    (doc) =>
      new Document({
        ...
        metadata: {
          ...
          sourceNormalized: (doc.metadata.source || '').toLowerCase().trim(),
...
export async function searchDocuments(query, k = 3, sessionId = 'default', userId = null, sources = null) {
  ...
  try {
    ...
    // Fallback: if source filter returned no results, try without filter and filter in JS
    // This handles legacy data that doesn't have sourceNormalized field
    if (results.length === 0 && sources && sources.length > 0) {
      console.log(`⚠️ No results with sourceNormalized filter, trying fallback...`);
      ...
      // Filter by source in JavaScript (case-insensitive)
      const normalizedSources = sources.map(s => s.toLowerCase().trim());
      results = fallbackResults.filter(([doc]) => {
        const docSource = (doc.metadata.source || '').toLowerCase().trim();
        return normalizedSources.some(s => docSource.includes(s) || s.includes(docSource));
      });
