...
export async function addDocumentsToStore(documents, sessionId = 'default', userId = null, waitForIndexing = true) {
  ...
  // CRITICAL: Wait for Pinecone eventual consistency
  // Serverless Pinecone doesn't make vectors immediately searchable after upsert
  // We need to poll/wait until vectors are indexed (typically 1-3 seconds)
  // See: https://github.com/pinecone-io/pinecone-ts-client/issues/226
  if (waitForIndexing && langchainDocs.length > 0) {
    const MAX_WAIT_ATTEMPTS = 8;  // Increased from 5 to 8 for more reliability
    const POLL_INTERVAL_MS = 500; // Faster polls (500ms instead of 800ms)
    const sampleContent = langchainDocs[0].pageContent.substring(0, 100); // More content for better matching

    // CRITICAL FIX: Also filter by the specific source being added
    // Without this, we might find OTHER vectors in the session and think indexing is done
    const sourceNormalized = langchainDocs[0].metadata?.sourceNormalized;

    console.log(`⏳ Waiting for Pinecone to index ${langchainDocs.length} vectors (source: ${sourceNormalized})...`);

    for (let attempt = 1; attempt <= MAX_WAIT_ATTEMPTS; attempt++) {
      await new Promise(resolve => setTimeout(resolve, POLL_INTERVAL_MS));

      try {
        // FIXED: Filter by source to ensure we're finding the NEWLY added vectors
        const filter = { sessionId, userId };
        if (sourceNormalized) {
          filter.sourceNormalized = sourceNormalized;
        }

        const results = await store.similaritySearchWithScore(sampleContent, 1, filter);

        if (results.length > 0) {
          // Double-check we found the right source
          const foundSource = results[0][0]?.metadata?.sourceNormalized;
          if (!sourceNormalized || foundSource === sourceNormalized) {
            console.log(`✅ Pinecone indexed vectors after ${attempt * POLL_INTERVAL_MS}ms (found: ${foundSource})`);
            break;
          } else {
            console.log(`⏳ Attempt ${attempt}/${MAX_WAIT_ATTEMPTS}: Found different source (${foundSource}), waiting for ${sourceNormalized}...`);
          }
        } else {
          console.log(`⏳ Attempt ${attempt}/${MAX_WAIT_ATTEMPTS}: Vectors not yet searchable...`);
        }
      } catch (error) {
        console.log(`⏳ Attempt ${attempt}/${MAX_WAIT_ATTEMPTS}: Error polling - ${error.message}`);
      }

      if (attempt === MAX_WAIT_ATTEMPTS) {
        console.log(`⚠️ Max wait time reached (${MAX_WAIT_ATTEMPTS * POLL_INTERVAL_MS}ms). Vectors may take a few more seconds to become searchable.`);
      }
    }
  }
